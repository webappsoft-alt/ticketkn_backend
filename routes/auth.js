...
router.post("/:type?", async (req, res) => {
  if (req.params.type == "social-login") {
    const { email, fcmtoken, name, type, dateOfBirth, gender } = req.body;
    console.log(email, fcmtoken, name, type, dateOfBirth, gender);
    const lowerCaseEmail = String(email).trim().toLocaleLowerCase();
    ...
    if (user.type !== type)
      return res.status(400).send({
        success: false,
        message: `You are not authorized to login as ${type}`,
      });
